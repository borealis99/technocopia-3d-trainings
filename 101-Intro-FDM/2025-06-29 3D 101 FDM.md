...
Students are encouraged to install PrusaSlicer on their personal computers.
# The Basics
...
<!-- ![alt text](image.png) -->
![alt text](./assets/cone-bunny-lines-demo.gif)

## The First Layer

![alt text](assets/line-types-first-layer.png)

## The Next Layers: Infill

![alt text](assets/line-types-simple.png)

## Overhangs, Support, Bridges
### Overhangs
As the shape of the layers changes, the positions of the lines will shift relative to the lines beneath them. When the lines move inward, they are supported by the infill beneath them. When the lines move outward, however, they can begin to "hang over the edge" of the layer beneath. This is fine to a certain extent -- more than you'd think -- but eventually we will need to supplement the part in progress with a supporting structure.

![cross section of layers, each further "off the edge" than the layer below](assets/progressive-overhang.png)

### Support
![simple support with IDEX](assets/support-cura-simple.png)
#### Tree Support
Modern slicers can cut down on material and printing time while improving surface finish by using "Tree" (or "Organic") supports.
![tree vs normal support](assets/tree-support-normal-support.png)

### Bridges

An exception to this rule is when the unsupported line makes contact with another supported area at its destination. This is known as bridging between the two supported areas. On a good machine with high quality filament, bridges can be printed with minimal deformation ("drooping"). The amount of deformation and risk of failure increases with the length of the bridge, so keep bridges as short as possible.

![two bridging test prints, one perfect and one with significant drooping](assets/bridging-ii.png)

### The Y-H-T Rule
![`Diagram showing three 3D print geometries and their support requirements. Left: Y-shape labeled "Shallow Overhang — Not Needed." Center: H-shape labeled "Short Bridge — Optional." Right: T-shape labeled "Sharp Overhang — Required."`](assets/Y-H-T-white.png)

## Strength

Contrary to what you might think at first, adhesion between layers is not formed using gravity. In fact, you can print upside down, as seen with experimental and ultra-compact printers such as the Positron. The geometry of the nozzle actually "squishes" plastic into the heated bed and into the previous layer.

![alt text](assets/extrusion_approx-01-01-1-800x450.png)

parts are weakest vertically -- they'll split between layers if force is applied vertically 

![alt text](assets/blue-yellow-red.jpg)
...
- **PLA** is the easiest to print, but parts are brittle will melt in a hot car. Best for decorative prints.
- **ABS** prints are strong and resilient, but is difficult to print. 
- **PETG** is a good balance for most applications, but its glossy surface can be displeasing to some.
...
There are multiple ways to safely clear the nozzle of debris.
- Pinch the nozzle with an inverted pair of flush cutters ("snips")
- Brush gently with a clean wire brush
- **DO NOT** use your fingers. I've seen it happen here. I don't care how already \[ messed up / burned / scarred \] your fingertips are.
